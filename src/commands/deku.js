...
module.exports = {
  ...
  async execute(sock, message, args, user, isGroup, groupData, reply) {
    ...
    try {
      ...
      const selectedFile = ImageRotationSystem.getNextImage(imageTracker, 'deku', files);
      ...
      await sock.sendMessage(senderJid, {
        image: imageBuffer,
        caption: isGroup ? MessageFormatter.elegantBox('💚 𝔇𝔈𝔎𝔘 💚', [{ label: '✨ Récompense', value: allowXp ? '+2 XP' : '🚫 Limite atteinte (10/jour)' }]) : MessageFormatter.elegantBox('💚 𝔇𝔈𝔎𝔘 💚', [{ label: '📺 Type', value: 'Personnage' }])
      });
