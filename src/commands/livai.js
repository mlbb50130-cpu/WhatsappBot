...
module.exports = {
  name: 'livai',
  description: 'Photo aléatoire de Livai',
  category: 'ASSETS',
  usage: '!livai',
  adminOnly: false,
  groupOnly: false,
  ...
  async execute(sock, message, args, user, isGroup, groupData, reply) {
    ...
    try {
      ...
      const selectedFile = ImageRotationSystem.getNextImage(imageTracker, 'livai', files);
      ...
      await sock.sendMessage(senderJid, {
        image: imageBuffer,
        caption: isGroup ? MessageFormatter.elegantBox('⚔️ 𝔏𝔦𝔳𝔞𝔦 ⚔️', [{ label: '✨ Récompense', value: allowXp ? '+2 XP' : '🚫 Limite atteinte (10/jour)' }]) : MessageFormatter.elegantBox('⚔️ 𝔏𝔦𝔳𝔰𝔦 ⚔️', [{ label: '📺 Type', value: 'Personnage' }])
      });
